...
const fetchSwagger = (done) => {
  ...
  const DEFAULT_SWAGGER = '../account-info-api-spec/dist/account-info-swagger.yaml';
  if (swagger && swagger.startsWith('https')) {
    log(`http get: ${swagger}`);
    const file = './swagger.yaml';
    fs.writeFileSync(file, '');

    https.get(swagger, (res) => {
  ...
  } else if (swagger && swagger.endsWith('.yaml') && fs.existsSync(swagger)) {
    done(swagger);
  } else if (fs.existsSync(DEFAULT_SWAGGER)) {
    done(DEFAULT_SWAGGER);
  } else {
